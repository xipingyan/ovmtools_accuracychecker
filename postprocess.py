...
for (i, name, new_fps, base_fps) in result_sets:
    ...
    outputA = subprocess.run([f'{ov_bench_dir}/benchmark_app', '-dopt', '-m', model_base_dir + name] + args, capture_output=True)
    ...
    outputA = subprocess.run([f'{ov_bench_dir}/benchmark_app', '-cpu_experimental=brgconv', '-dopt', '-m', model_base_dir + name] + args, capture_output=True)
    out = outputA.stdout.decode()
    if outputA.returncode == 0:
